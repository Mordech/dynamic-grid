...
describe('DynamicGrid', () => {
  ...
  it('should the render props correctly', () => {
    ...
    expect(
      ...
      `"display: grid; --dg-min-width-grid-shrink: min(100%, var(--dg-min-width, 200px)); gap: var(--dg-gap, unset); overflow: auto hidden; scroll-behavior: smooth; scrollbar-width: thin; scroll-snap-type: x mandatory; grid-auto-flow: column; grid-template-rows: repeat(var(--dg-scroll-rows, 1), 1fr); grid-auto-columns: max((100% - (var(--dg-repeat-count, 4) + var(--dg-scroll-hint, 0) - 1) * var(--dg-gap-inline, var(--dg-gap, 0px))) / (var(--dg-repeat-count, 2) + var(--dg-scroll-hint, 0)), var(--dg-min-width-grid-shrink, var(--dg-min-width, 200px))); --dg-repeat-count: 2; --dg-min-width: 20px; --dg-gap: 10px; --dg-gap-inline: 10px; --dg-scroll-hint: 0.5; visibility: visible; pointer-events: auto; background-color: rgba(0, 0, 0, 0); border-block-start-color: CanvasText; border-block-end-color: CanvasText; border-inline-start-color: CanvasText; border-inline-end-color: CanvasText; caret-color: auto;"`,
    );
  });
